...
class ChannelController extends Controller
{
    ...
    /**
     * $sport: 0-football, 1-ice hockey, 2-basketball, 3-tennis, 4-baseball, 5-golf, 6-bicycle racing, 7-racing, 8-others
     * 
     */
    public function getlivematch($sport) {
    	$rootUrl = 'http://livetv.sx/export/webmasters.php?lang=en&s=' . $sport;
    	$html = $this->file_get_html($rootUrl);
    	$result = [];
    	foreach ($html->find('table') as $table) {
	    	foreach ($table->find('table') as $match) {
	    		$match = $this->str_get_html($match);
				$m = array();
	    		$m['time'] = trim($match->find('td', 0)->innertext);
	    		$m['league'] = trim($match->find('td', 2)->find('span', 0)->innertext);
	    		$m['name'] = trim(strip_tags($match->find('td', 3)->innertext));
	    		$href = $match->find('td', 3)->find('a', 0);
	    		$m['href'] = ($href != null) ? 'http://livetv.sx' . $href->href : $href;
				$result[] = $m;
			}
    	}
