...
@if (Session::has('youtubeVideoId'))
	Last upload...{{ session('youtubeVideoId') }}
@endif

@if (isset($authUrl))
	<a href="{{ $authUrl }}">Connect me</a>
@elseif (isset($accessToken))
	The access token has been added to your database: <b>{{ $accessToken }}</b> now <a href="/admin/youtubeupload">try an upload</a>
@else
	{{ Form::open(array('url' => '/admin/youtubeupload', 'class' => 'form', 'files' => true)) }}
